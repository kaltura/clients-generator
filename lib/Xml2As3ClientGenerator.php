...
class Xml2As3ClientGenerator extends ClientGeneratorFromXml
{
	...
	private function createVoClass( $xml )
	{
		...
		///////////////////////////////////////////////
		if($xml->attributes()->base)
			$str .= "		override public function getElementType(arrayName:String):String\n";
		else {
			if($this->generateDocs)
			{
				$str .= "		/** \n";
				$str .= "		* get the expected type of array elements \n";
				$str .= "		* @param arrayName \t name of an attribute of type array of the current object \n";
				$str .= "		* @return \t un-qualified class name \n";
				$str .= "		**/ \n";
			}
			$str .= "		public function getElementType(arrayName:String):String\n";
		}

		$str .="		{\n";
		$str .="			var result:String = '';\n";
		$str .="			switch (arrayName) {\n";
		...
		foreach($xml->children() as $child)
		{
			switch($child->attributes()->type)
			{
				...
				default :
					// keep required type 
					$str .= "				case '" . $child->attributes()->name . "':\n";
					$str .= "					result = '" . $child->attributes()->arrayType . "';\n";
					$str .= "					break;\n";
					break;
			}
		}
		...
		{
			$str .= "				default:\n";
			$str .= "					result = super.getElementType(arrayName);\n";
			$str .= "					break;\n";
		}

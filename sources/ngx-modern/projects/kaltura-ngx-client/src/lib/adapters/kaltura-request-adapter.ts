        ...
        error => {
          if (environment.response.customErrorInHttp500) {
            if (error && typeof error.error === 'string') {
              ...
              return throwError(() => new KalturaAPIException(actualError.message, actualError.code, actualError.args));
            }
          ...
          return throwError(() => new KalturaClientException("client::request-network-error", errorMessage || 'Error connecting to server'));
        }
      ),
      map(
        result => {

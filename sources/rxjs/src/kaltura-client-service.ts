...
export class KalturaClient {
  ...
  public request<T>(request: KalturaRequest<T> | KalturaFileRequest, format?: KalturaResponseType): Observable<T | { url: string }> {
    ...
    if (typeof format !== "undefined") {
      return new KalturaRequestAdapter().transmit(request, this._options, this._defaultRequestOptions, format + "");
    }

    if (request instanceof KalturaFileRequest) {
      return observableThrowError("KalturaFileRequestAdapter is not supported");
      ...
      return observableThrowError("KalturaUploadRequest is not supported");

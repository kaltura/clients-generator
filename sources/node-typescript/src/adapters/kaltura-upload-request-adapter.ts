...
import { KalturaLogger } from '../api/kaltura-logger';
import got from 'got';
...
const logger = new KalturaLogger('KalturaUploadRequestAdapter');
      ...
      if (uploadChunkData.enabled) {
        ...
        if (userChunkFileSize && Number.isFinite(userChunkFileSize) && !Number.isNaN(userChunkFileSize)) {
          if (userChunkFileSize < 1e5) {
            ...
            logger.info(`using user requested chunk size '${userChunkFileSize}'`);
            actualChunkFileSize = userChunkFileSize;
          }
        } else {
          logger.info(`using default chunk size 5Mb`);
        ...
        logger.info(`chunk upload not supported by browser or by request. Uploading the file as-is`);
      }

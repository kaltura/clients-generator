...
public class APIOkRequestsExecutor implements RequestQueue {
	...
	protected ResponseElement onGotResponse(Response response, RequestElement action) {
        ...
        if (!response.isSuccessful()) { // in case response has failure status
            return new ExecutedRequest().requestId(requestId).headers(response.headers()).error(ErrorElement.fromCode(response.code(), response.message())).success(false);
            ...
            return new ExecutedRequest().requestId(requestId).response(responseString).headers(response.headers()).code(response.code()).success(responseString != null);
        }
    }

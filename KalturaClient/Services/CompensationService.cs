// ===================================================================================================
//                           _  __     _ _
//                          | |/ /__ _| | |_ _  _ _ _ __ _
//                          | ' </ _` | |  _| || | '_/ _` |
//                          |_|\_\__,_|_|\__|\_,_|_| \__,_|
//
// This file is part of the Kaltura Collaborative Media Suite which allows users
// to do with audio, video, and animation what Wiki platfroms allow them to do with
// text.
//
// Copyright (C) 2006-2018  Kaltura Inc.
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as
// published by the Free Software Foundation, either version 3 of the
// License, or (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <http://www.gnu.org/licenses/>.
//
// @ignore
// ===================================================================================================
using System;
using System.Xml;
using System.Collections.Generic;
using System.IO;
using Kaltura.Request;
using Kaltura.Types;
using Kaltura.Enums;

namespace Kaltura.Services
{
	public class CompensationAddRequestBuilder : RequestBuilder<Compensation>
	{
		#region Constants
		public const string COMPENSATION = "compensation";
		#endregion

		public Compensation Compensation
		{
			set;
			get;
		}

		public CompensationAddRequestBuilder()
			: base("compensation", "add")
		{
		}

		public CompensationAddRequestBuilder(Compensation compensation)
			: this()
		{
			this.Compensation = compensation;
		}

		public override Params getParameters(bool includeServiceAndAction)
		{
			Params kparams = base.getParameters(includeServiceAndAction);
			if (!isMapped("compensation"))
				kparams.AddIfNotNull("compensation", Compensation);
			return kparams;
		}

		public override Files getFiles()
		{
			Files kfiles = base.getFiles();
			return kfiles;
		}

		public override object Deserialize(XmlElement result)
		{
			return ObjectFactory.Create<Compensation>(result);
		}
	}

	public class CompensationDeleteRequestBuilder : RequestBuilder<object>
	{
		#region Constants
		public const string ID = "id";
		#endregion

		public long Id
		{
			set;
			get;
		}

		public CompensationDeleteRequestBuilder()
			: base("compensation", "delete")
		{
		}

		public CompensationDeleteRequestBuilder(long id)
			: this()
		{
			this.Id = id;
		}

		public override Params getParameters(bool includeServiceAndAction)
		{
			Params kparams = base.getParameters(includeServiceAndAction);
			if (!isMapped("id"))
				kparams.AddIfNotNull("id", Id);
			return kparams;
		}

		public override Files getFiles()
		{
			Files kfiles = base.getFiles();
			return kfiles;
		}

		public override object Deserialize(XmlElement result)
		{
			return null;
		}
	}

	public class CompensationGetRequestBuilder : RequestBuilder<Compensation>
	{
		#region Constants
		public const string ID = "id";
		#endregion

		public long Id
		{
			set;
			get;
		}

		public CompensationGetRequestBuilder()
			: base("compensation", "get")
		{
		}

		public CompensationGetRequestBuilder(long id)
			: this()
		{
			this.Id = id;
		}

		public override Params getParameters(bool includeServiceAndAction)
		{
			Params kparams = base.getParameters(includeServiceAndAction);
			if (!isMapped("id"))
				kparams.AddIfNotNull("id", Id);
			return kparams;
		}

		public override Files getFiles()
		{
			Files kfiles = base.getFiles();
			return kfiles;
		}

		public override object Deserialize(XmlElement result)
		{
			return ObjectFactory.Create<Compensation>(result);
		}
	}


	public class CompensationService
	{
		private CompensationService()
		{
		}

		public static CompensationAddRequestBuilder Add(Compensation compensation)
		{
			return new CompensationAddRequestBuilder(compensation);
		}

		public static CompensationDeleteRequestBuilder Delete(long id)
		{
			return new CompensationDeleteRequestBuilder(id);
		}

		public static CompensationGetRequestBuilder Get(long id)
		{
			return new CompensationGetRequestBuilder(id);
		}
	}
}
